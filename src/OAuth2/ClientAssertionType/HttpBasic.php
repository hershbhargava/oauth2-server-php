...
class HttpBasic implements ClientAssertionTypeInterface
{
    ...
    public function getClientCredentials(RequestInterface $request, ResponseInterface $response = null)
    {
        ...
        if ($this->config['allow_credentials_in_request_body']) {
            ...
            if (!is_null($request->query('client_id', $request->request('client_id')))) {
                /**
                 * client_secret can be null if the client's password is an empty string
                 * @see http://tools.ietf.org/html/rfc6749#section-2.3.1
                 */

                return array('client_id' => $request->query('client_id', $request->request('client_id')), 'client_secret' => $request->query('client_secret', $request->request('client_secret')));
            }
        }
